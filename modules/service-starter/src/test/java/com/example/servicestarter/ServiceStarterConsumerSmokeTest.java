...
@Testcontainers
@SpringBootTest(classes = ServiceStarterConsumerSmokeTest.ServiceStarterApplication.class)
class ServiceStarterConsumerSmokeTest {

    @Container
    private static final MySQLContainer MYSQL = new MySQLContainer("mysql:8.4.10")
            .withDatabaseName("service_starter")
            .withUsername("service_starter")
            .withPassword("service_starter")
    ...
    @Test
    @DisplayName("Service starter는 DB 정보만으로 Web과 JPA와 MySQL 기반을 시작한다")
    void startsServiceStarterWithDatabaseConfiguration() {
        // given
    ...
    @Test
    @DisplayName("Service starter는 서비스 migration만 실행하고 인증 테이블을 만들지 않는다")
    void runsServiceMigrationWithoutAuthenticationTable() {
        // given
        String serviceTable = "service_starter_probe";
    ...
    static class ServiceStarterApplication {
    }
}
