package com.example.msagatewaystarter;
...
@SpringBootTest(
        classes = MsaGatewayWebMvcConsumerSmokeTest.MsaGatewayWebMvcApplication.class,
        properties = "jwt.secret=test-secret-key-that-is-at-least-32-characters-long"
)
@AutoConfigureMockMvc
class MsaGatewayWebMvcConsumerSmokeTest {
    ...
    @Test
    @DisplayName("MSA Gateway starter는 Spring Data 없이 Web MVC 컨텍스트를 시작한다")
    void startsMsaGatewayStarterWithoutSpringData() {
        // given
    ...
    @Test
    @DisplayName("보호된 요청의 인증 실패를 표준 ProblemDetail로 반환한다")
    void unauthenticatedProtectedRequestReturnsProblemDetail() throws Exception {
        // given
        String endpoint = CURRENT_MEMBER_PATH;
        ...
        assertThat(result.getResponse().getContentAsString())
                .contains("\"type\":\"/problems/authentication-required\"");
    }
    ...
    void validJwtAccessesProtectedEndpointWithStringSubject() throws Exception {
        ...
        MvcResult result = mockMvc.perform(
                        get(CURRENT_MEMBER_PATH).header("Authorization", "Bearer " + accessToken)
                )
                .andExpect(status().isOk())
        ...
        // then
        assertThat(result.getResponse().getStatus()).isEqualTo(200);
        assertThat(result.getResponse().getContentAsString()).contains(SUBJECT.value());
    }

    @Test
    @DisplayName("권한이 부족한 인증 요청을 접근 거부 ProblemDetail로 반환한다")
    void insufficientRoleReturnsAccessDeniedProblemDetail() throws Exception {
        // given
        String accessToken = accessToken(new AuthenticationRole("MEMBER"));
        ...
        assertThat(result.getResponse().getContentAsString())
                .contains("\"type\":\"/problems/access-denied\"");
    }
    ...
    void swaggerEndpointIsNotPublicByDefault() throws Exception {
        ...
        // when
        MvcResult result = mockMvc.perform(get(swaggerEndpoint))
                .andExpect(status().isUnauthorized())
                .andReturn();
    ...
    static class MsaGatewayWebMvcApplication {
    }
