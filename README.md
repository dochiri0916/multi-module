...
다만 사용자 도메인 자체는 포함하지 않는다. `User`, `Member`, `Account` 같은 엔티티와 repository는 소비 프로젝트가 직접 가져야 한다.
...
- `dochiri-security-jpa`는 `User` 엔티티를 제공하지 않는다.
- 사용자 도메인 모델과 로그인 비즈니스는 소비 프로젝트가 직접 정의한다.
- `security-jpa`는 `refresh_tokens` 테이블과 `userId` 기반 refresh token 저장만 담당한다.
- 따라서 `RefreshToken`은 `@ManyToOne User` 연관관계 대신 `Long userId`만 저장한다.
- refresh token은 삭제보다 폐기 개념을 사용하므로 `deletedAt`이 아니라 `revokedAt`을 가진다.
- 즉 refresh token 레코드는 남겨 두고, 더 이상 사용할 수 없게 된 시점을 `revokedAt`으로 기록한다.
...
- `revoked_at`: refresh token 폐기 시각. `null`이면 아직 폐기되지 않은 상태다.

`revokedAt`을 쓰는 이유:

- 로그아웃, 재발급, 강제 만료 같은 무효화 시점을 기록할 수 있다.
- refresh token 레코드를 바로 지우지 않아도 "왜 더 이상 못 쓰는지"를 표현할 수 있다.
- 나중에 만료되거나 오래된 폐기 토큰을 별도 배치로 물리 삭제하는 것도 가능하다.
